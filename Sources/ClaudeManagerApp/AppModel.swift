...
final class AppModel: ObservableObject {
    ...
    ///
    @Published private(set) var pendingRestart: [String: Int32] = [:]

    /// Replace the restart-nudge map (`private(set)`, so `AppModel+RestartNudge` drives it
    /// via this rather than every write in the module being able to reach it).
    func setPendingRestart(_ value: [String: Int32]) {
    ...
    /// Latest usage per **binding** id (a launcher path, or `TokenBinding.defaultID`); one Claude
    /// login shared across profiles appears under each of its bindings, so a view keyed by profile
    /// reads it directly.
    ...
    /// Bumped **only when tracking is toggled off** (`applyUsageTrackingChange`'s disable branch),
    /// to disown the pass in flight; an off→on sequence is covered too, since it necessarily passes
    /// through that off. A pass suspended on its `await` compares this after resuming and, if it
    /// changed, commits nothing and leaves the single-flight state to whoever owns the current
    /// generation — so a cancelled pass can't repopulate state the toggle-off just cleared, nor
    /// hold the lock shut.
