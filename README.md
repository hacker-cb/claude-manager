...
- **Stays in the menu bar** — closing the window leaves Claude Manager running in the
  menu bar; reopen it from the menu bar icon (or the Dock icon, when shown), and quit with
  ⌘Q. **Settings → Startup** adds optional **Launch at login** and **Menu bar only** (hide
  the Dock icon) — both off by default.
